...
// On Vercel, the @vercel/fastify builder handles HTTP routing and manages the
// server lifecycle — calling listen() would block the event loop waiting for a
// port that Vercel already controls.  Outside Vercel (local dev, Docker, etc.)
// we start the server as usual.
