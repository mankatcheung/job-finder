...
describe('ResetPasswordUseCase', () => {
  ...
  it('throws UNAUTHORIZED when no token matches the hash', async () => {
    ...
    const err = await new ResetPasswordUseCase({ userRepository, passwordResetTokenRepository })
  ...
  it('updates the password and marks the token used for a valid token', async () => {
    ...
    await new ResetPasswordUseCase({ userRepository, passwordResetTokenRepository }).execute({
      token: RAW_TOKEN,
