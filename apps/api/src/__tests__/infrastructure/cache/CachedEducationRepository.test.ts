...
describe('CachedEducationRepository', () => {
  ...
  describe('delete', () => {
    it('delegates to inner and invalidates byId and list caches', async () => {
      ...
      await repo.delete('edu-1', 'user-1');
    ...
    it('evicts the list cache from a second repo instance with no prior read on it (JEF-209 — the id→userId map this used to depend on is gone)', async () => {
      const inner = makeEducationRepository();
      const cache = new MemoryCache(60_000);
      const repoA = new CachedEducationRepository({ drizzleEducationRepository: inner, cache });
      const repoB = new CachedEducationRepository({ drizzleEducationRepository: inner, cache });
      ...
      await repoB.delete('edu-1', 'user-1');
