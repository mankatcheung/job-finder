...
import { user, passwordResetToken } from '#src/infrastructure/db/schema.js';
...
describe('DrizzlePasswordResetTokenRepository', () => {
  ...
  beforeAll(async () => {
    ...
    repo = new DrizzlePasswordResetTokenRepository({ db: db.db });
  });

  afterAll(() => db.cleanup());

  beforeEach(async () => {
    await db.db.delete(passwordResetToken);
