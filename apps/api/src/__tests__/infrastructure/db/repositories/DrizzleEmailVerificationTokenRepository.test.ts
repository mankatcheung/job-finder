...
import { user, emailVerificationToken } from '#src/infrastructure/db/schema.js';
...
describe('DrizzleEmailVerificationTokenRepository', () => {
  ...
  beforeAll(async () => {
    ...
    repo = new DrizzleEmailVerificationTokenRepository({ db: db.db });
  });

  afterAll(() => db.cleanup());

  beforeEach(async () => {
    await db.db.delete(emailVerificationToken);
