...
describe('PrismaTotpBackupCodeRepository', () => {
  let db: TestDb;
  let repo: PrismaTotpBackupCodeRepository;

  beforeAll(async () => {
    db = await createTestDb();
    repo = new PrismaTotpBackupCodeRepository({ prisma: db.prisma });
  });

  afterAll(() => db.cleanup());

  beforeEach(async () => {
    await db.prisma.totpBackupCode.deleteMany();
