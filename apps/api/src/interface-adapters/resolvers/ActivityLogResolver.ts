...
import type { ActivityLogMapper, ActivityLogDTO } from '@/interface-adapters/mappers/ActivityLogMapper.js';
...
export class ActivityLogResolver {
  ...
  async getActivityLogs(applicationId: string, ctx: GraphQLContext): Promise<ActivityLogDTO[]> {
    ...
    const logs = await this.deps.getActivityLogsUseCase.execute({ applicationId, userId: ctx.user.sub });
