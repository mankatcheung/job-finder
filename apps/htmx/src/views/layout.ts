...
function navItem(href: string, label: string, active: boolean, icon: string): string {
  const cls = active
...
function bottomNavItem(href: string, label: string, active: boolean, icon: string): string {
  const cls = active
...
const icons = {
  ...
  applications: `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="14" x="2" y="7" rx="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>`,
  ...
  account: `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="4"/><path d="M20 21a8 8 0 1 0-16 0"/></svg>`,
...
export function layout(content: string, title: string, activeNav: ActiveNav): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  ...
  <div class="min-h-screen flex">
    ...
    <header class="lg:hidden fixed top-0 inset-x-0 z-40 h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4">
      ...
      <form action="/logout" method="POST">
        <button type="submit" aria-label="Sign out" class="p-2 text-gray-500 hover:text-gray-900 rounded-lg transition-colors">
          ${icons.logout}
        </button>
      </form>
    </header>
    ...
    <aside class="hidden lg:flex w-60 flex-shrink-0 bg-white border-r border-gray-200 flex-col fixed top-0 left-0 h-full">
      ...
      <nav class="flex-1 px-3 py-4 space-y-1">
        ${navItem('/dashboard', 'Dashboard', activeNav === 'dashboard', icons.dashboard)}
        ${navItem('/applications', 'Applications', activeNav === 'applications', icons.applications)}
        ${navItem('/analytics', 'Analytics', activeNav === 'analytics', icons.analytics)}
      </nav>
      <div class="px-3 pb-2 space-y-1">
        ${navItem('/account', 'Account', activeNav === 'account', icons.account)}
      </div>
      <div class="px-3 py-4 border-t border-gray-200">
        <form action="/logout" method="POST">
          <button type="submit" class="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors">
            ${icons.logout}
            Sign out
          </button>
        </form>
    ...
    <nav class="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-white border-t border-gray-200 flex">
      ${bottomNavItem('/dashboard', 'Dashboard', activeNav === 'dashboard', icons.dashboard)}
      ${bottomNavItem('/applications', 'Apps', activeNav === 'applications', icons.applications)}
      ${bottomNavItem('/analytics', 'Analytics', activeNav === 'analytics', icons.analytics)}
      ${bottomNavItem('/account', 'Account', activeNav === 'account', icons.account)}
    </nav>
  </div>
</body>
</html>`;
...
export function authLayout(content: string, title: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
