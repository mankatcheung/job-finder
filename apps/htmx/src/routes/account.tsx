...
function TokenRow({ t, newToken }: { t: Token; newToken?: string }) {
  return (
    <div
      id={`token-${t.id}`}
    ...
    >
      <div>
        <p class="text-sm font-medium text-gray-900" safe>
          {t.name}
        </p>
        <p class="text-xs text-gray-400" safe>
          {t.scope} · Created {formatDate(t.createdAt)}
      ...
      <button
        class="shrink-0 ml-2 px-2 py-1 text-xs text-red-600 border border-red-200 hover:bg-red-50 rounded-lg transition-colors"
      ...
      <div class="p-4 sm:p-8 max-w-2xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <form action="/account/password" method="POST" class="space-y-3">
            <div>
              <label class={labelCls} for="currentPassword">
                Current password
              </label>
              <input
                id="currentPassword"
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <p class="text-xs text-gray-500 mb-4">
            Read-scoped tokens can be used with the MCP server for AI agent access.
          </p>
        ...
        <div class="bg-white rounded-xl border border-red-200 p-5">
          ...
          <form
            ...
            method="POST"
            onsubmit="return confirm('Delete your account? All data will be lost.')"
          >
...
async function loadPage(
  ...
  return <AccountPage email={meData.me.email} tokens={tokensData.apiTokens} flash={flash} />;
}
...
export default async function accountRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/account/tokens', async (request, reply) => {
    ...
    try {
      ...
      return reply.type('text/html').send(<TokenRow t={rest} newToken={rawToken} />);
    } catch (err) {
      if ((err as Error).message === 'Redirecting') return;
