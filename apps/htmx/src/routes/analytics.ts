...
function bar(label: string, count: number, total: number, color: string): string {
  const pct = total > 0 ? Math.round((count / total) * 100) : 0;
  return `
    <div class="flex items-center gap-2">
      <span class="text-xs text-gray-600 w-24 shrink-0 capitalize">${label}</span>
      <div class="flex-1 bg-gray-100 rounded-full h-3">
        <div class="h-3 rounded-full transition-all" style="width:${pct}%;background-color:${color}"></div>
      </div>
      ...
      <div class="p-4 sm:p-8 max-w-4xl mx-auto">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div class="space-y-2">
              ${ALL_STATUSES.map((s) => bar(s, byStat[s] ?? 0, total, STATUS_COLORS[s] ?? '#9ca3af')).join('')}
            </div>
          ...
          <!-- Weekly activity -->
          <div class="bg-white rounded-xl border border-gray-200 p-5">
            <h2 class="text-base font-semibold text-gray-900 mb-4">Applications per week (last 12 weeks)</h2>
                ...
                .map(([week, count]) => {
                  ...
                  return `
                    <div class="flex-1 flex flex-col items-center gap-1 group relative">
                      <div class="w-full rounded-t bg-blue-400 hover:bg-blue-500 transition-colors cursor-default" style="height:${h}px" title="${week}: ${count}"></div>
                      <span class="text-xs text-gray-400" style="font-size:9px">${label}</span>
                    </div>`;
                })
                .join('')}
            </div>
          ...
          <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div class="space-y-3">
              ...
              <div class="flex items-center justify-between text-sm">
                ...
                <span class="font-semibold">${interviewing} <span class="text-xs text-gray-400">(${convRate(interviewing, applied)}%)</span></span>
              </div>
              <div class="flex items-center justify-between text-sm">
                <span class="text-gray-600">Offered</span>
                <span class="font-semibold">${offered} <span class="text-xs text-gray-400">(${convRate(offered, applied)}%)</span></span>
              </div>
              <div class="flex items-center justify-between text-sm">
                <span class="text-gray-600">Accepted</span>
                <span class="font-semibold">${byStat['accepted'] ?? 0} <span class="text-xs text-gray-400">(${convRate(byStat['accepted'] ?? 0, applied)}%)</span></span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div class="space-y-3">
              ...
              <div class="flex items-center justify-between text-sm">
                ...
                <span class="font-semibold">${apps.filter((a) => a.starred).length} (${starRate}%)</span>
              </div>
              <div class="flex items-center justify-between text-sm">
