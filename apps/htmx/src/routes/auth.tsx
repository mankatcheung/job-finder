      ...
      <div class="w-full max-w-sm">
        <div class="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
          ...
          <form action="/login" method="POST" class="space-y-4">
            ...
            <div>
              <label class={labelCls} for="password">
                Password
              </label>
              <input
                id="password"
...
function RegisterPage({ error }: { error?: string }) {
  return (
  ...
  fastify.post('/login', async (request, reply) => {
    ...
    try {
      ...
      if (json.errors?.length) {
        return reply
          .type('text/html')
          .send(<LoginPage error={json.errors[0]?.message ?? 'Login failed'} />);
      }
  ...
  fastify.post('/register', async (request, reply) => {
    ...
    try {
      ...
      if (json.errors?.length) {
        return reply
          .type('text/html')
          .send(<RegisterPage error={json.errors[0]?.message ?? 'Registration failed'} />);
      }
      const setCookies = res.headers.getSetCookie?.() ?? [];
      for (const c of setCookies) {
        void reply.header('set-cookie', c);
      ...
      return reply
        .type('text/html')
        .send(<RegisterPage error="Something went wrong. Please try again." />);
    }
  });
